        ...
        <div class="bg-white border-[#e8e8e8] border-[0.5px] border-solid flex flex-col gap-[18px] items-stretch overflow-hidden px-[18px] py-[12px] md:px-[36px] md:py-[20px] relative rounded-[14px] shadow-[0px_0px_0px_0.5px_rgba(64,37,0,0.15)] w-full">
            ...
            <form action="{{ route('public.submit', $form->slug) }}" method="POST" id="main-form" @submit="onSubmit($event)" class="relative z-10 flex flex-col gap-[18px]">
                    ...
                    @foreach($form->fields as $field)
                        ...
                        <div @if($hasLogic) x-show="evaluateLogic({{ json_encode($field->logic) }})" x-transition x-cloak @endif>
                            ...
                            @elseif($field->type === 'button')
                                @php
                                    $btnText = $field->config['button_text'] ?? null;
                                    $btnUrl  = $field->config['button_url']  ?? '#';
                                @endphp
                                @if($btnText)
                                <a href="{{ $btnUrl }}" target="_blank"
                                   class="w-full relative flex items-center justify-center gap-[9px] px-[13.5px] py-[10.5px] border-[0.75px] border-[rgba(232,232,232,0.9)] rounded-[6px] text-black font-medium text-[14px] tracking-[-0.154px] overflow-hidden shadow-[inset_0px_1.5px_1.5px_0px_rgba(255,255,255,0.05)] hover:brightness-95 active:brightness-90 transition-all"
                                   style="background-image: linear-gradient(180deg, #ffffff 0%, #ededed 100%);">
                                    <span class="relative z-10">{{ $btnText }}</span>
                                </a>
                                @endif
                            @elseif($field->type === 'wa_button')
                                <!-- wa_button rendered below submit -->
                            @else
                                <!-- Input Questions Row -->
                                <div class="flex flex-col gap-1.5 py-1.5 border-b border-[#e8e8e8]/30 last:border-0 pb-2">
                                    ...
                                    @if(!empty($field->config['wa_button_text']))
                                    <a href="{{ $field->config['wa_button_url'] ?? '#' }}" target="_blank"
                                       class="mt-1 w-full relative flex items-center justify-center gap-[9px] px-[13.5px] py-[10.5px] border-[0.75px] border-[rgba(232,232,232,0.9)] rounded-[6px] text-black font-medium text-[14px] tracking-[-0.154px] overflow-hidden shadow-[inset_0px_1.5px_1.5px_0px_rgba(255,255,255,0.05)] hover:brightness-95 active:brightness-90 transition-all"
                                       style="background-image: linear-gradient(180deg, #ffffff 0%, #ededed 100%);">
                                        <span class="relative z-10">{{ $field->config['wa_button_text'] }}</span>
                ...
                <div class="mt-2 flex flex-col gap-3 relative z-10">
                    ...
                    <!-- Render wa_button types here -->
                    @foreach($form->fields as $field)
                        @if($field->type === 'wa_button')
                            @php
                                $hasLogic = !empty($field->logic) && is_array($field->logic);
                                $btnText = $field->config['wa_button_text'] ?? null;
                                $btnUrl  = $field->config['wa_button_url']  ?? '#';
                            @endphp
                            @if($btnText)
                            <div @if($hasLogic) x-show="evaluateLogic({{ json_encode($field->logic) }})" x-transition x-cloak @endif class="w-full relative z-10">
                                <a href="{{ $btnUrl }}" target="_blank"
                                   class="w-full relative flex items-center justify-center gap-[9px] px-[13.5px] py-[10.5px] border-[0.75px] border-[rgba(232,232,232,0.9)] rounded-[6px] text-black font-medium text-[14px] tracking-[-0.154px] overflow-hidden shadow-[inset_0px_1.5px_1.5px_0px_rgba(255,255,255,0.05)] hover:brightness-95 active:brightness-90 transition-all"
                                   style="background-image: linear-gradient(180deg, #ffffff 0%, #ededed 100%);">
                                    <span class="relative z-10">{{ $btnText }}</span>
