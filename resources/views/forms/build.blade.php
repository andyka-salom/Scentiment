<x-app-sidebar-layout title="Builder — {{ $form->title }}">
    ...
    <div x-data="builderApp()" x-init="initBuilder()">
        <div class="grid grid-cols-12 gap-5 items-start">
                ...
                <div class="col-span-12 md:col-span-4 lg:col-span-3 bg-white rounded-2xl border border-slate-100 p-5 shadow-sm static md:sticky md:top-32 z-10">
                            ...
                            <div :data-id="field.id" class="p-0 rounded-xl border bg-white shadow-sm flex flex-col transition relative group" :class="{'ring-2 ring-slate-900 border-transparent my-6': selectedField && selectedField.id === field.id, 'border-slate-100 hover:border-slate-200': !(selectedField && selectedField.id === field.id)}" @click="if(!selectedField || selectedField.id !== field.id) selectField(field)">
                                ...
                                <div class="p-5 flex-1 w-full" :class="{'pl-10': !(selectedField && selectedField.id === field.id)}">
                                    ...
                                    <div x-show="!selectedField || selectedField.id !== field.id">
                                        ...
                                        <!-- Preview WA Button (if configured) -->
                                        <div x-show="field.config && field.config.wa_button_text" class="mt-4">
                                            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 shadow-sm">
                                                ...
                                                <span x-text="field.config.wa_button_text"></span>
                                            </div>
                                        </div>
                                            ...
                                            <div class="mt-4 border border-green-100 rounded-xl overflow-hidden">
                                                <button type="button" @click="selectedField._showWa = !selectedField._showWa"
                                                    ...
                                                    <span class="flex items-center gap-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-600" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                                                        Tombol WhatsApp (Opsional)
                                                    </span>
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform" :class="selectedField._showWa ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                                                </button>
                                                <div x-show="selectedField._showWa" class="p-4 space-y-3 bg-white">
                                                    <p class="text-xs text-slate-500">Tambahkan tombol WhatsApp yang muncul di bawah pertanyaan ini pada form.</p>
                                                    <div>
                                                        <label class="block text-xs font-semibold text-slate-600 mb-1">Teks Tombol</label>
                                                        <input type="text" x-model="selectedField.config.wa_button_text" @input="updateFieldDebounced(selectedField)" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" placeholder="Contoh: Customer Care Heaven Scent">
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-semibold text-slate-600 mb-1">Nomor / URL WhatsApp</label>
                                                        <input type="text" x-model="selectedField.config.wa_button_url" @input="updateFieldDebounced(selectedField)" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" placeholder="Contoh: https://wa.me/628123456789">
                                                    </div>
                                                    <button x-show="selectedField.config.wa_button_text" @click="selectedField.config.wa_button_text = ''; selectedField.config.wa_button_url = ''; updateField(selectedField)" type="button" class="text-xs text-rose-500 hover:text-rose-700 font-medium">Hapus Tombol WA</button>
                                                </div>
                                            </div>
                ...
                addField(type) {
                    fetch('{{ route("fields.store", $form) }}', {
                        ...
                        body: JSON.stringify({
                            ...
                            type: type,
                            is_required: false,
                            config: type === 'scale' ? { scale_min: 1, scale_max: 5 } : { wa_button_text: '', wa_button_url: '' },
